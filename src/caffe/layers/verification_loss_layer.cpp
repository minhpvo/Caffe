...
template <typename Dtype>
void VerificationLossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype> *> &bottom, const vector<Blob<Dtype> *> &top)
{
  ...
  {
    dist_sq_.mutable_cpu_data()[i] = caffe_cpu_dot(channels, diff_.cpu_data() + (i * channels), diff_.cpu_data() + (i * channels));
...
template <typename Dtype>
void VerificationLossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype> *> &top, const vector<bool> &propagate_down, const vector<Blob<Dtype> *> &bottom)
{
  Dtype margin = this->layer_param_.contrastive_loss_param().margin();
  bool legacy_version = this->layer_param_.contrastive_loss_param().legacy_version();
  for (int i = 0; i < 2; ++i)
    ...
    if (propagate_down[i])
    {
      ...
      for (int j = 0; j < num; ++j)
      {
        ...
        if (static_cast<int>(bottom[2]->cpu_data()[j]) == static_cast<int>(bottom[3]->cpu_data()[j])) // similar pairs
          caffe_cpu_axpby(channels, alpha, diff_.cpu_data() + (j * channels), Dtype(0.0), bout + (j * channels));
        else // dissimilar pairs
        {
          Dtype mdist(0.0);
          Dtype beta(0.0);
          ...
          if (mdist > Dtype(0.0))
            caffe_cpu_axpby(channels, beta, diff_.cpu_data() + (j * channels), Dtype(0.0), bout + (j * channels));
          else
...
REGISTER_LAYER_CLASS(VerificationLoss);
